...
namespace Site.BLL.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<OperationDetails> Update(UserDTO userDto)
        {
            ...
            return new OperationDetails(true,"User blocked","");
        ...
        public async Task<OperationDetails> Create(UserDTO userDto)
            {
                ...
                if (user == null)
                {
                    ...
                    await Database.SaveAsync();
                    return new OperationDetails(true, "Register success", "");
                }
                else
                {
                    return new OperationDetails(false, "This login is already in use", "Email");
                }
            }
        ...
        {
            IEnumerable<ApplicationUser> UserDB = Database.ClientManager.GetAll();
            var mapper = new MapperConfiguration(c => c.CreateMap<ApplicationUser, UserDTO>()).CreateMapper();
            var users = mapper.Map<IEnumerable< ApplicationUser >,List <UserDTO>>(UserDB);
            for (int i = 0; i < users.Count();i++)
            {
                users[i].Name = Database.ClientManager.GetProfiles().Where(c => c.Id == users[i].Id).FirstOrDefault().Name;
            }
            return users;
        }
        public void Dispose()
